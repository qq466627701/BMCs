                                 ...
                                 std::string& version)
{
    ...
    //遍历每个activeendpoints
    {
        ...
        // 与ipmi::getDbusProperty同样获取ec error
    ...
    ipmiAppGetDeviceId(ipmi::Context::ptr ctx)
{
    static struct
    {
        ...
        uint7_t fwMajor;
        bool devBusy;//设备忙碌
        uint8_t fwMinor;//软件镜像
        uint8_t ipmiVer = 2;//ipmi版本 lanplus
        uint8_t addnDevSupport;//addn dev 支持
        uint24_t manufId;//菜单fid
        uint16_t prodId;//生产ID
        uint32_t aux;//串口？？
    } devId;
    ...
    if (!fwVerInitialized)//软件版本未能初始化
    {
        std::string versionString;//版本字符串
        //获取有效软件版本
                                          ...
                                          versionString))
        {
            ...
            if (rev.has_value())
            {
                //@ rev.value()返回类型为MetaRevision
                MetaRevision revision = rev.value();
                devId.fwMajor = static_cast<uint7_t>(revision.major);

                revision.minor = (revision.minor > 99 ? 99 : revision.minor);   //三元表达 99或minor
                devId.fwMinor =
                    revision.minor % 10 + (revision.minor / 10) * 16;// 10进制转16进制？？？
                try
                {
